#include "GD3800_Serial.h"
 ...
 void  GD3800_Serial::play()
 {
  this->sendCommand(0x01);
}

void  GD3800_Serial::restart()
{
...
void  GD3800_Serial::pause()
{
  this->sendCommand(0x02);
}

void  GD3800_Serial::next()
{
  this->sendCommand(0x03);
}

void  GD3800_Serial::prev()
{
  this->sendCommand(0x04);
}
void  GD3800_Serial::fastReverse()
{
  this->sendCommand(0x0B);
}

void  GD3800_Serial::fastForward()
{
  this->sendCommand(0x0A);
}

void  GD3800_Serial::playFileByIndexNumber(unsigned int fileNumber)
{  
  this->sendCommand(0x41, (fileNumber>>8) & 0xFF, fileNumber & (byte)0xFF);
}

void  GD3800_Serial::PlayFilesByFileName(unsigned int fileNumber)
{  
  this->sendCommand(0x49,(fileNumber>>8) & 0xFF, fileNumber & (byte)0xFF);
}

void  GD3800_Serial::PushPlayFileByIndexNumber(unsigned int fileNumber)
{  
  this->sendCommand(0x43, (fileNumber>>8) & 0xFF, fileNumber & (byte)0xFF);
}

void  GD3800_Serial::volumeUp()
{
  this->sendCommand(0x05);
}

void  GD3800_Serial::volumeDn()
{
  this->sendCommand(0x06);
}

void  GD3800_Serial::setVolume(byte volumeFrom0To30)
{
  this->sendCommand(0x31, volumeFrom0To30);
}

void  GD3800_Serial::setEqualizer(byte equalizerMode)
{
  this->sendCommand(0x32, equalizerMode);
}

void  GD3800_Serial::setLoopMode(byte loopMode)
{
  this->sendCommand(0x33, loopMode);
}


void  GD3800_Serial::setDevice(byte device)
{
...
byte  GD3800_Serial::getStatus()    
{
  ...
  do
  {
    ...
    for(byte x = 0; x < MP3_STATUS_CHECKS_IN_AGREEMENT; x++)
    {
      ...
      byte  GD3800_Serial::getVolume()    { return this->sendCommandWithUnsignedIntResponse(0x11); }
      byte  GD3800_Serial::getEqualizer() { return this->sendCommandWithUnsignedIntResponse(0x44); }
      byte  GD3800_Serial::getLoopMode()  { return this->sendCommandWithUnsignedIntResponse(0x13); }
    
      unsigned int  GD3800_Serial::countFiles()   
      {
       
          return this->sendCommandWithUnsignedIntResponse(0x16); 
      }

      unsigned int  GD3800_Serial::currentFileIndexNumber()
      {
    ...
    unsigned int GD3800_Serial::sendCommandWithUnsignedIntResponse(byte command)
    {      
      char buffer[5];
    ...
    void  GD3800_Serial::sendCommand(byte command)
    {
      this->sendCommand(command, 0, 0, 0, 0);
    }
    
    void  GD3800_Serial::sendCommand(byte command, byte arg1)
    {
     this->sendCommand(command, arg1, 0, 0, 0);
   }

   void  GD3800_Serial::sendCommand(byte command, byte arg1, byte arg2)
   {
     this->sendCommand(command, arg1, arg2, 0, 0);
   }    

   void  GD3800_Serial::sendCommand(byte command, byte arg1, byte arg2, char *responseBuffer, unsigned int bufferLength)
   {
...
size_t GD3800_Serial::readBytesUntilAndIncluding(char terminator, char *buffer, size_t length, byte maxOneLineOnly)
{
...
int GD3800_Serial::waitUntilAvailable(unsigned long maxWaitTime)
{
